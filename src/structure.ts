...
const TYPE_SCORES: Record<EntityType, number> = {
  institution: 6,
  company: 5,
  person: 4,
  product: 3,
...
function cleanEntityToken(token: string): string {
...
function isSubstringOfTitle(name: string, title: string): boolean {
...
function hasContext(content: string, name: string, patterns: string[]): boolean {
  const escaped = escapeRegExp(name);
  return patterns.some((pattern) => new RegExp(pattern.replace('%ENTITY%', escaped), 'i').test(content));
}

function isSimpleCapitalizedToken(token: string): boolean {
...
function isSuspiciousMergedToken(token: string): boolean {
...
function isCamelCaseToken(token: string): boolean {
  return /^[A-Z][a-z]+[A-Z][A-Za-z]*[,]?$/.test(token) && !isSuspiciousMergedToken(token);
}

function isEntityLikeToken(token: string): boolean {
...
function isEntityStartToken(token: string): boolean {
  const cleaned = cleanEntityToken(token);
...
function isPersonWord(word: string): boolean {
...
function classifyEntity(name: string, document: ExtractedDocument, count: number): EntityType {
  ...
  if (
    /^[A-Z][A-Za-z.-]+(?:\s+[A-Z][A-Za-z.-]+)*(?:,\s*[A-Z][A-Za-z.-]+)?$/.test(normalized)
  ...
  if (
    (productLike && genericWordCount === 0)
  ) {
...
function collectEntityCandidates(document: ExtractedDocument): EntityCandidate[] {
...
function dedupeEntities(entities: Entity[]): Entity[] {
  const exact = new Map<string, Entity>();
...
function extractEntities(document: ExtractedDocument, topics: string[]): Entity[] {
  const primaryEntities = dedupeEntities(
    collectEntityCandidates(document)
      .map((candidate) => ({
  ...
  const topicEntities = topics
    ...
    .map((topic) => ({ name: topic, type: 'topic' as const }));
...
function buildTopicRelationships(nodes: KnowledgeNode[]): Map<string, Relationship[]> {
  ...
  for (let index = 0; index < nodes.length; index += 1) {
    ...
    for (let otherIndex = index + 1; otherIndex < nodes.length; otherIndex += 1) {
      ...
      const sharedEntities = right.entities.filter(
        (entity) => entity.type !== 'topic' && leftEntities.has(`${entity.type}:${normalizeEntityKey(entity.name)}`)
      );
...
function mergeRelationships(...groups: Relationship[][]): Relationship[] {
