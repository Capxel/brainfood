...
function renderBlockChildren($, element) {
    if (element.type === 'text') {
        return $(element).text().replace(/\s+/g, ' ');
    }
    if (element.type !== 'tag') {
...
function extractFromHtml(document) {
    ...
    const title = normalizeWhitespace(article?.title || document.titleHint || $('title').text() || $('h1').first().text() || 'Untitled');
    const author = normalizeWhitespace(article?.byline || document.authorHint || extractMetaContent($, ['meta[name="author"]', 'meta[property="article:author"]']) || '') || null;
    const publishedAt = safeIsoDate(document.publishedAtHint) ||
...
function extractFromText(document) {
...
export function extractDocument(rawDocument) {
    if (rawDocument.html) {
